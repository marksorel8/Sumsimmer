...
us<-unstructured(4)
 ...
 nll <- nll -sum(dexp(sigma,1,log=TRUE))+sum(log_sigma)
 nll
}
...
in_riv_pred |> 
   ggplot(aes(x=in_river_allowed_HR,y=pred,color=Sector))+geom_line(lwd=1.25)+geom_point(data=pfmc_morts_2,aes(x=in_river_allowed_HR,y=in_river_HR,color=Sector),cex=1.5)+geom_abline(intercept=0,slope=1)+geom_line(aes(x=in_river_allowed_HR,y=lcl),lty=2)+geom_line(aes(x=in_river_allowed_HR,y=ucl),lty=2)+facet_wrap(~Sector)
...
pdat |> ggplot(aes(y=pred,x=NOS,color=factor(hatchery_escapement)))+geom_line(lwd=2)+facet_wrap(~population_name,scales="free_x")+scale_color_discrete(type=blues9[-1])
