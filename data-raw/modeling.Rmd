...
FIXME:: This all needs to be redone to reflect the new RTMB model form
...
# test_mod<-glmmTMB::glmmTMB(lrps~basin-1+S:basin +ar1(year_fac+0|const)+diag(basin+0|year_fac),data=test_mod_dat,dispformula = ~0)
# 
# test_mod<-glmmTMB::glmmTMB(lrps~wild+S:basin +us(basin+0|year_fac),data=test_mod_dat,dispformula = ~0)
...
test_mod<-glmmTMB::glmmTMB(lrps~1+wild+S:basin +us(basin+0|year_fac),data=test_mod_dat,dispformula = ~0)


 summary(test_mod)

 eval<-test_mod$frame |> as_tibble() |> mutate(pred= predict(test_mod,re.form=NA),resid=pred-lrps)
 forecast::Acf(eval |> filter(basin=="Methow") |> pull(resid))
  forecast::Acf(eval |> filter(basin=="Wenatchee") |> pull(resid))
  forecast::Acf(eval |> filter(basin=="Okanogan") |> pull(resid))
 forecast::Acf(eval |> filter(basin=="Hatchery") |> pull(resid))
...
# for ( i in 1:4){
#   proc_err[i,,]<-(rnorm(n_years*n_sims,0,sigma_unique[i])+shared)
# }
# 
#   return(proc_err)
#   }
...
pfmc_mort_dat|> ggplot(aes(x=log(RMRS),y=log(PFMCNT)))+geom_point()+geom_smooth(method="lm",formula=("y~log(x)-1"))
...
pred<-predict(pfm_mort_mod,newdata=data.frame(RMRS),se=TRUE,interval="prediction")
  
pred$fit |> bind_cols(RMRS=RMRS)  |> 
  ggplot(aes(x=RMRS))+geom_ribbon(aes(ymin = (lwr),ymax=(upr)),fill="grey")+geom_line(aes(y=(fit)))+geom_point(data=pfmc_mort_dat,aes(y=PFMCNT))+theme_classic()+ylab("PFMC AEQ")+xlab("River mouth run size")
...
pfmc_mort_dat |> ggplot(aes(x=run_size,y=(`PFMC NT Ocean Impacts`)))+geom_point()+geom_smooth(method="lm")


pfm_ER_mod<-lm(qlogis(`PFMC NT Ocean Impacts`)~1,data=pfmc_mort_dat)
AIC(pfm_ER_mod)
summary(pfm_ER_mod)
...
pred_harv<-(stats::predict(pfm_ER_mod,newdata=data.frame(RMRS=seq(1000,150000,by=10000))))

pred_harv_lcl<-plogis(pred_harv+(1.96*sigma(pfm_ER_mod)))
pred_harv_lcl<-((pred_harv_lcl)*seq(1000,150000,by=10000))/(1-(pred_harv_lcl))

pred_harv_ucl<-plogis(pred_harv-(1.96*sigma(pfm_ER_mod)))
pred_harv_ucl<-((pred_harv_ucl)*seq(1000,150000,by=10000))/(1-(pred_harv_ucl))

pred_harv_l50<-plogis(pred_harv+(.67*sigma(pfm_ER_mod)))
pred_harv_l50<-((pred_harv_l50)*seq(1000,150000,by=10000))/(1-(pred_harv_l50))

pred_harv_u50<-plogis(pred_harv-(.67*sigma(pfm_ER_mod)))
pred_harv_u50<-((pred_harv_u50)*seq(1000,150000,by=10000))/(1-(pred_harv_u50))



plot(pfmc_mort_dat$RMRS,pfmc_mort_dat$`PFMC NT Ocean Impacts`,pch=19,ylab="PFMC NT AEQ HR",xlab="Rivermouth run size")
points(seq(1000,150000,by=10000),pred_harv,type="l")
points(seq(1000,150000,by=10000),pred_harv,type="l")

#/(pred_harv+seq(1000,150000,by=10000)),type="l")

# averave prop of RMRS
PFMC_ave_prop<-coef(pfm_ER_mod)

# calculate the standard deviation of the log accuracy ratio
sd_lar_pfmc<-sigma(pfm_ER_mod)
  
  mean(abs(log((predict(pfm_mort_mod))/pfmc_mort_dat$PFMCNT)))
#
mean(log(exp(predict(pfm_mort_mod))/pfmc_mort_dat$PFMCNT))

plot(seq(1000,150000,by=10000),(plogis(pred_harv)*seq(1000,150000,by=10000))/(1-plogis(pred_harv)),type="l",ylab="PFMC NT AEQ morts",xlab="Rivermouth run size",ylim=c(0,20000))



polygon(c(seq(1000,150000,by=10000),rev(seq(1000,150000,by=10000))),
         c(pred_harv_lcl,rev(pred_harv_ucl)),col="lightgrey",border="lightgrey")

polygon(c(seq(1000,150000,by=10000),rev(seq(1000,150000,by=10000))),
        c(pred_harv_l50,rev(pred_harv_u50)),col="grey",border="grey")

points(seq(1000,150000,by=10000),plogis(pred_harv)*seq(1000,150000,by=10000),type="l")
points(pfmc_mort_dat$RMRS,pfmc_mort_dat$PFMCNT,pch=19)




points(seq(1000,150000,by=10000),(plogis(pred_harv_lcl)*seq(1000,150000,by=10000))/(1-plogis(pred_harv_lcl)),type="l",lty=3)
points(seq(1000,150000,by=10000),(plogis(pred_harv_ucl)*seq(1000,150000,by=10000))/(1-plogis(pred_harv_ucl)),type="l",lty=3)
...
points(seq(1000,150000,by=10000),plogis(plogis(pred_harv)+1*sigma(pfm_ER_mod)),type="l",lty=2)
points(seq(1000,150000,by=10000),plogis(pred_harv+(-1*sd_lar_pfmc)),type="l",lty=2)
points(seq(1000,150000,by=10000),exp(log(pred_harv)+(1.96*sd_lar_pfmc)),type="l",lty=3)
points(seq(1000,150000,by=10000),exp(log(pred_harv)+(-1.96*sd_lar_pfmc)),type="l",lty=3)
n_year<-50
n_sim<-500
set.seed(910)
pfmc_err<-matrix(rnorm(n_year*n_sim,0,sd_lar_pfmc),n_year,n_sim)
...
fram_db <- connect_fram_db("data-raw/data/Valid2022_Round_7.1.1_11142023.mdb")
...
PFMCNTAEQ<-mortality |> filter(fishery_id%in%c(12, 16, 18, 20, 22, 26, 27, 30, 31, 32, 33, 34, 35),stock_id%in%c(45,46)) |> left_join(AEQ,relationship = "many-to-one") |> 
  mutate(tot_AEQ_mort= (landed_catch +non_retention +shaker+drop_off  +
           msf_landed_catch +msf_non_retention +msf_shaker+msf_drop_off)*aeq ) |> 
  left_join(fram_db |> 
  fetch_table('RunID') |> select(run_id,run_year),relationship="many-to-one") |> group_by(run_year) |> summarize(AEQ_morts=sum(tot_AEQ_mort)) 
    
    
   PFMCNTAEQ |>  ggplot(aes(x=run_year,y=AEQ_morts))+geom_point()+geom_line()
 
tac_RR |> pivot_longer(Age3:Age6,names_to="Age",values_to="riv_mouth") |>
  mutate(age=as.numeric(substr(Age,4,4)),
         year=BroodYear+age) |> group_by(year) |> summarise(rmrs=sum(riv_mouth)) |>
  left_join(PFMCNTAEQ |> mutate(year=as.numeric(run_year))) |>
  mutate(er=AEQ_morts/(AEQ_morts+rmrs )) |> ggplot(aes(x=AEQ_morts+rmrs,y=er))+geom_point()
```
...
```{r term_HR_actual}
pfmc_morts_2<-pfmc_morts |> select(`Allowed Treaty or NT Harvest`,`PFMC NT Harvest Total`,`Total In-river NT`,`Total Treaty`,`Total NT`,RMRS=`River Mouth Run Size`,Year) |> pivot_longer(c(`Total Treaty`,`Total In-river NT`),values_to = "In_river_harvest",names_to = "Sector") |>
...
##in river
pfmc_morts_2[-c(15,16,24),]|> 
  ggplot(aes(x=Allowed_in_river,y=(In_river_harvest)))+geom_point()+geom_smooth(method="lm",formula=("y~(x)-1"),se=FALSE)+
  facet_wrap(~Sector)+ylab("In-rive harvest")+xlab("Allowed in river")+geom_abline(intercept=0,slope=1,lwd=1.5)


pfmc_morts_2|>
  ggplot(aes(x=(in_river_allowed_HR),y=(in_river_HR)))+geom_point()+geom_abline(intercept=0,slope=1,lwd=1.5)+#geom_smooth(method="lm",formula=("y~(x)"),se=FALSE)#+
  facet_wrap(~Sector)+ylim(0,.3)+xlim(0,.3)+ylab("In-rive HR")+xlab("Allowed in-river HR")#+geom_smooth(method="lm",formula=("y~(x)-1"),se=FALSE)
...
in_riv_mod<-lm(log(Total_harvest)~Sector:log(total_allowed)-1,data=pfmc_morts_2[-c(15,16,24,22,23,25),])

in_riv_mod<-glmmTMB::glmmTMB(log(Total_harvest)~Sector:log(total_allowed)-1+diag(0+Sector|Year),data=pfmc_morts_2[-c(15,16,24,22,23,25),],dispformula = ~0)
...
in_riv_pred<-data.frame(total_allowed=rep(seq(1,100000,1000),times=2), Sector=rep(c("NT","Treaty"),each=100),Year=1)
...
in_riv_pred |> 
   ggplot(aes(x=total_allowed,y=pred,color=Sector))+geom_ribbon(aes(x=total_allowed,ymax = ucl,ymin=lcl),fill="grey")+geom_line(lwd=1.2)+geom_point(data=pfmc_morts_2,aes(x=total_allowed,y=Total_harvest))+geom_abline(intercept=0,slope=1)+ylim(-10000,50000)+xlim(0,50000)+facet_wrap(~Sector)+xlab("Total *allowed* harvest")+ylab("Estimated total harvest")


### save to internal data for model option 1`

in_river_harvest_model_coefs
...
in_riv_pred<-data.frame(#Allowed_in_river=rep(seq(0,100000,5000),times=1),
                        in_river_allowed_HR=rep(seq(0,.5,.01),times=2),
 Sector=as.factor(rep(c("NT","Treaty"),each=51)))
in_riv_pred$pred=plogis(predict(lin_mod,newdata=in_riv_pred))
in_riv_pred$lcl=plogis(predict(lin_mod,newdata=in_riv_pred)-1.96*sigma(lin_mod))
in_riv_pred$ucl=plogis(predict(lin_mod,newdata=in_riv_pred)+1.96*sigma(lin_mod))


in_riv_pred |> 
   ggplot(aes(x=in_river_allowed_HR,y=pred,color=Sector))+geom_ribbon(aes(x=in_river_allowed_HR,ymax = ucl,ymin=lcl),fill="grey")+geom_line(lwd=1.2)+geom_point(data=pfmc_morts_2,aes(x=in_river_allowed_HR,y=in_river_HR))+geom_abline(intercept=0,slope=1)+ylim(-10000,100000)+xlim(0,50000)+facet_wrap(~Sector)
...
in_riv_pred$pred=plogis(c(mod_fun(x=qlogis(seq(0,.5,.01)),seg_fit_NT$par[1],seg_fit_NT$par[2]),mod_fun(x=qlogis(seq(0,.5,.01)),seg_fit_T$par[1],seg_fit_T$par[2])))



in_riv_pred$ucl<-plogis(qlogis(in_riv_pred$pred)+2*exp(ifelse(in_riv_pred$Sector=="Treaty",seg_fit_T$par[3],seg_fit_NT$par[3])))
in_riv_pred$lcl<-plogis(qlogis(in_riv_pred$pred)-2*exp(ifelse(in_riv_pred$Sector=="Treaty",seg_fit_T$par[3],seg_fit_NT$par[3])))


in_riv_pred |> 
   ggplot(aes(x=in_river_allowed_HR,y=pred))+geom_line(lwd=1.25,color="firebrick4")+geom_point(data=pfmc_morts_2,aes(x=in_river_allowed_HR,y=in_river_HR),cex=1.75)+geom_abline(intercept=0,slope=1)+geom_line(aes(x=in_river_allowed_HR,y=lcl),lty=2,color="firebrick4")+geom_line(aes(x=in_river_allowed_HR,y=ucl),lty=2,color="firebrick4")+facet_wrap(~Sector)+ylim(.1,.35)+xlim(0,.4)+ylab("In-rive HR")+xlab("Allowed in-river HR")
...
# are the annual errors highly correlated among sectors... nope
pfmc_morts_2 |> dplyr::select(Sector,resid,RMRS) |> pivot_wider(values_from = resid,names_from = Sector) |>  dplyr::select(-RMRS) |> cor(use="pairwise.complete.obs")

pfmc_morts_2 |> group_by(Sector) |>  summarize(sd_lar=mean((log(pred/in_river_HR))))
set.seed(1112)
...
esc_dat %>% filter(year>=2012)|> dplyr::select(year,population_name ,Broodstock=NOBroodStockRemoved,NOR) %>%
  drop_na()  %>% ggplot(aes(x=(NOR),y=(Broodstock)))+geom_point()+facet_wrap(~population_name,scales="free_x")+geom_smooth(method = "lm",formula="(y)~log(x)")+xlab("Natural-origin escapement")+ylab("Natural-origin broodstock")
...
esc_dat %>% filter(!is.na(`SG No. Age 4`)) |> filter(year>=2010) |>  dplyr::select(year,population_name ,PHOS,HOS,NOS) %>% left_join((returns |> group_by(year) |> summarize(hatch_return=sum(hatchery))) |> 
  drop_na())|> 
  drop_na()  %>% ggplot(aes(x=hatch_return,y=HOS))+geom_point()+facet_wrap(~population_name,scales="free_x")+theme(legend.position = "top")+geom_smooth(method="lm")

phos_dat |>  dplyr::select(year,population_name ,PHOS,HOS,NOS) %>% left_join((returns |> group_by(year) |> summarize(hatch_return=sum(hatchery))) |> 
  drop_na())|> 
  drop_na()  %>% ggplot(aes(x=(NOS),y=log(HOS)))+geom_point()+facet_wrap(~population_name,scales="free_x")+theme(legend.position = "top")+geom_smooth(method="lm",formula="y~x")
...
phos_dat<-esc_dat%>% filter(!is.na(`SG No. Age 4`)) |> filter(year>=2010) |> mutate(logit_phos=qlogis(PHOS)) |> left_join((returns |> group_by(year) |> summarize(hatch_return=sum(hatchery))) |> 
 ...
 phos_dat%>% filter(!is.na(`SG No. Age 4`)) |> filter(year>=2010) |>  dplyr::select(year,population_name ,PHOS,NOS,hatchery_escapement) %>%
  drop_na()  %>% ggplot(aes(x=NOS,y=PHOS,color=hatchery_escapement))+geom_point(cex=3)+facet_wrap(~population_name,scales="free_x")+theme(legend.position = "top")+geom_smooth(method="lm",formula = "y~log(x)")+ylab("Proportion hatchery origin spawners (pHOS)")+xlab("Natural origin spawners")

phos_dat %>% filter(!is.na(`SG No. Age 4`)) |> filter(year>=2010) |>  
ggplot(aes(x=hatchery_escapement,y=qlogis(PHOS)))+geom_point()+facet_wrap(~population_name,scales="free_x")+theme(legend.position = "top")+geom_smooth(method="lm")
...
pHOS_mod<-glmmTMB::glmmTMB(log(HOS)~population_name+(log(NOS))-1+(population_name+0|year_fac),data=phos_dat,dispformula = ~0)
summary(pHOS_mod)
AIC(pHOS_mod)
...
phos_dat |> 
  ...
  dplyr::select(population_name, NOS = value_sequence) |> expand_grid(hatchery_escapement=seq(5000,150000,by=20000),year_fac=as.factor(3000))

pred<-predict(pHOS_mod,newdata = pdat)

pdat<-pdat |> mutate(pred=exp(pred)) 

pdat |> ggplot(aes(y=pred,x=NOS))+geom_line(lwd=2)+facet_wrap(~population_name,scales="free_x")+ylab("Predicted pHOS")+xlab("Natural origin spawners")+geom_point(data=phos_dat,aes(x=NOS,y=HOS))

# pdat<-pdat |> mutate(pred=plogis(pred)) 
pdat |> mutate(hatchery_escapement=as.factor(hatchery_escapement))|> ggplot(aes(y=pred,x=NOS,color=hatchery_escapement))+geom_line(lwd=2)+facet_wrap(~population_name,scales="free_x")+scale_color_discrete(type=blues9[-1])+ylab("Predicted pHOS")+xlab("Natural origin spawners")+geom_point(data=phos_dat,aes(x=NOS,y=PHOS,color=year_fac))
...
URR |> ggplot(aes(x=Year,y=URR))+geom_point()+geom_line()+xlim(2008,2023)+ylab("Unmarked release rate")+geom_segment(aes(x=2013,y=internal_data$URR,xend=2023,yend=internal_data$URR),color="firebrick4",lty=2,lwd=1.5)
...
log_RPS<-test_mod_dat |> select(brood_year,basin,lrps) |> 
  pivot_wider(values_from = lrps, names_from = basin) |> 
  arrange(brood_year)


 scale_log_RPS<-log_RPS |> select(-brood_year) |> 
  # mutate(across(everything(),scale)) |>
  as.matrix() |> 
  t()
 
 # 
 # spawner_cov<-test_mod_dat |> select(brood_year,basin,S) |> 
 #  pivot_wider(values_from = S, names_from = basin) |> 
 #  arrange(brood_year) |> filter(brood_year>=1996) |> select(-brood_year) |> 
 #  # mutate(across(everything(),scale)) |> 
 #  as.matrix() |> 
 #  t()
...
# mutate(across(everything(),scale)) |> 
  ...
  t()
#-----------------------------------------------
mus<-log_RPS |> select(-brood_year) |>   summarise(across(everything(),mean,na.rm=T)) 
 sds<-log_RPS|> select(-brood_year) |>   summarise(across(everything(),sd,na.rm=T)) 
 
 library(MARSS)
 ns<-nrow(scale_log_RPS)
n_fac<-1

B=matrix(list(0),4,4)#"diagonal and unequal"
diag(B)<-.5list("M",0,"W",0)
B="diagonal and unequal"
Q="unconstrained"
# R=diag(4)*.01
R="diagonal and unequal"
D=matrix(list("M",0,0,0,0,"O",0,0,0,0,"W",0,0,0,0,0),4,4)
U="zero"
Z="identity"
A=matrix(list("a","a","a",0),4,1)
x0="unequal"
 model.list<-list(B=B,Q=Q,R=R,d=spawner_cov,D=D,U=U,Z=Z,A=A,x0=x0)
 
 fit <- MARSS(scale_log_RPS, model=model.list)
 
 fit_coef<-coef(fit)
 fit_coef$A*sds+mus
 fit_coef$D*sds[1:3]
### DFA### coefficients()DFA
