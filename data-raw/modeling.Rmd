...
sim_MREER |> ggplot(aes(x=cy,y=MREER))+geom_point()+facet_wrap(~Age)+geom_line()+geom_vline(aes(xintercept=2022))
...
if(sim_for_model){
  ...
  n_sims<-500
   ...
   sim_MREER_for_mod<-MARSS::MARSSsimulate(sim_surv_mod,tSteps = 50,n_sims)$sim.data  
   
   
  #first col is calendar year 2013
  MREER_out<-array(NA,dim=c(5,60,n_sims))
  MREER_out[,1:10,]<-fit.dfa.surv$ytT[,41:50]
  MREER_out[,11:60,]<-sim_MREER_for_mod
  MREER_out<-MREER_out*scales$sd+scales$mu
  MREER_out<-plogis(MREER_out)
  for (i in 2:5){
    for ( j in 1:n_sims){
      MREER_out[i,,j]<-dplyr::lead(c(MREER_out[i,,j]),(i-1))
    }
  }
  MREER_out<-apply(MREER_out,2:3,cumprod)
  
  dimnames(MREER_out)[1:2]<-list(age=2:6,Brood_year=2011:(2011+59))
...
W <- function(x) {
  ...
  beta_corrected<-beta*(alpha_corrected[-1]/(alpha[-1]))
   umsy <- (1 - W(exp(1-alpha_corrected[-1])))
   Smsy <- (1 - W(exp(1-alpha_corrected[-1]))) / beta_corrected
...
##total
pfmc_morts_2|> ggplot(aes(x=`Allowed Treaty or NT Harvest`,y=Total_harvest))+geom_point()+geom_smooth(method="lm",formula=("y~x"),se=FALSE)+geom_abline(intercept=0,slope=1,lwd=1.5)+facet_wrap(~Sector)
...
pfmc_morts_2|>
  ggplot(aes(x=qlogis(in_river_allowed_HR),y=qlogis(in_river_HR)))+geom_point()+geom_smooth(method="lm",formula=("y~(x)"),se=FALSE)+facet_wrap(~Sector)+geom_abline(intercept=0,slope=1,lwd=1.5)
...
in_riv_pred |> 
   ggplot(aes(x=Allowed_in_river,y=pred))+geom_line(color="red",lwd=1.2)+geom_point(data=pfmc_morts_2,aes(x=Allowed_in_river,y=In_river_harvest))+geom_abline(intercept=0,slope=1)+ylim(0,100000)


in_riv_mod_treaty<-lm(In_river_harvest~Allowed_in_river*Sector,data=pfmc_morts_2 )
AIC(in_riv_mod_treaty)
segmented_model <- segmented::segmented(in_riv_mod_treaty, seg.Z = ~Allowed_in_river,by=Sector)
AIC(segmented_model)


library(segmented)
segmented_model <-segmented::segreg(In_river_harvest~Sector+seg(Allowed_in_river),data=pfmc_morts_2 |> mutate(Sector=as.factor(Sector)))
...
in_riv_pred<-data.frame(Allowed_in_river=rep(seq(0,100000,5000),times=1),
 Sector=as.factor(rep(c("NT","Treaty"),each=21)))
in_riv_pred$pred=(predict(segmented_model,newdata=in_riv_pred))



in_riv_pred |> 
   ggplot(aes(x=Allowed_in_river,y=pred,color=Sector))+geom_line(lwd=1.25)+geom_point(data=pfmc_morts_2,aes(x=Allowed_in_river,y=In_river_harvest,color=Sector),cex=1.5)+geom_abline(intercept=0,slope=1)+ylim(0,100000)
...
pfmc_morts_2 |>ggplot(aes(y=log(pred/In_river_harvest),x=Allowed_in_river))+geom_point()+facet_wrap(~Sector)+geom_hline(yintercept=0)
...
Process for simulating harvest
- simulate PSMFC AEQ as a function of RMRS
- calculate allowable as a function of RMRS and rule set
- calculate allowable in-river based on total allowable of PSMFC
- simulate in-river as a function of allowable in-river

```{r}


sim_PSMFC<-function(RMRS,PFMC_ave_prop=PFMC_ave_prop,sd_lar_pfmc=sd_lar_pfmc){
  exp(PFMC_ave_prop*log(RMRS)+(rnorm(1,0,sd_lar_pfmc)))
}



sim_in_river<-function(allowed,PFMC,coefs,psi,sd_lar_in_river){
...
allowed_Treaty<-function(Run_size,tiers=c(16000,36250,50000),rates=c(.05,.1,.5,.5)){
...
esc_dat %>% filter(year>=2012)|> dplyr::select(year,population_name ,Broodstock=NOBroodStockRemoved,NOR) %>%
  drop_na()  %>% ggplot(aes(x=(NOR),y=(Broodstock)))+geom_point()+facet_wrap(~population_name,scales="free_x")+geom_smooth(method = "lm",formula="log(y)~log(x)")
...
# deciding whether to log transform NOS and hatchery returns could have considerable effect on oucome. Need to chat with managers to determine how to represent managment of pHOS. Key question is whether pHOS increases a lot at low NOS (log relationship) or not.
```
