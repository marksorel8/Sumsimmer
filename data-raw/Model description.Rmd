...
The expected number of fish handled that results in $n^\text{dead}$ dead fish is then $n^\text{Handle}=n^\text{dead}/\left(p^\text{Mark total}+(1-p^\text{Mark total})\phi\right)$. Assuming that all handled marked fish are retained, the expected number of dead marked fish is $n^\text{Dead marked}=n^\text{Handle}p^\text{Mark total}$, and the expected number of dead unmarked fish is $n^\text{dead unmarked}=n^\text{Handle}(1-p^\text{Mark total})\phi$.
