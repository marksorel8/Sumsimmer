...
pdat|> ggplot(aes(x=NOS,y=pred,color=hatchery_escapement))+geom_line(aes(group=group),lwd=2)+facet_wrap(~population_name,scales="free_x")+scale_color_gradient(low = "lightblue", high = "darkblue",limits=c(10000,80000))+ylab("pHOS")+xlab("1,000 natural origin spawners")+geom_point(data=phos_dat |> filter(!is.na(hatchery_escapement)),aes(x=NOS,y=PHOS,fill=hatchery_escapement),size=3,shape=21,color="black")+scale_fill_gradient(low = "lightblue", high = "darkblue",limits=c(10000,80000)) +theme_minimal()+scale_x_continuous(labels=function(x)x/1000)
...
add_baselines<-FALSE
if(add_baselines){
  internal_data$sim_current<-summarize_sim(pop_sim(),HCR="Current MA")
  internal_data$sim_0<-summarize_sim(pop_sim(treaty_tiers = NA,treaty_rates = 0,NT_tiers=NA,NT_rates=0),HCR="h0")
...
yrs<-7:31
  ...
  pfmc_morts<-
    internal_data$pfmc_morts_2 |> select(Year,Sector,In_river_harvest) |> pivot_wider(values_from = In_river_harvest,names_from = Sector) |> left_join(internal_data$pfmc_morts_2 |> select(Year,Sector,Total_harvest) |> filter(Sector=="NT") |> select(Year,NT_total_harvest=Total_harvest)) |>  filter(between(Year,2008,2023)) |>
