...
ricker_pred<-SR_TMB_dat$test_mod_dat %>% group_by(basin,const,mu_rps,sd_rps,mu_s,sd_s,wild) %>% reframe(S=seq(0,max(max(S_tot),4000)*1.5,length.out=50)) %>%
...
ricker_pred2 |> ggplot(aes(x=S))+geom_ribbon(aes(ymax = ucl,ymin=lcl),fill="grey")+
  facet_wrap(~basin,scales="free")+ geom_line(aes(y=recruits),color="blue",lwd=1.25)+geom_point(data=all_SR |> filter(brood_year>=1997) |> mutate(S=S_RRS) ,aes(y=R))+
...
if(sim_for_model){
  ...
  if(add_baselines){
    sim<-pop_sim()
    sim_list<-list(perf=summarize_sim(sim,HCR="Current MA"))#hcr=seq_HCR(),
