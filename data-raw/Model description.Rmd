...
pdat |> mutate(hatchery_escapement=as.factor(hatchery_escapement))|> ggplot(aes(x=NOS))+geom_line(aes(y=pred,color=hatchery_escapement),lwd=2)+facet_wrap(~population_name,scales="free_x")+scale_color_discrete(type=blues9[-1])+ylab("pHOS")+xlab("Natural origin spawners")+geom_point(data=phos_dat,aes(x=NOS,y=PHOS))
