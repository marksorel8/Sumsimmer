...
SR_TMB_dat2=list(
S=
all_SR |> filter(brood_year>=1997) |> 
  # mutate(S_tot=S_no+.8*S_ho) |> 
  select(brood_year,S_RRS,basin) |> 
  pivot_wider(names_from = basin,values_from = S_RRS) |> arrange((brood_year))|> arrange((brood_year)) |> 
...
SR_TMB_dat2$S[is.na(SR_TMB_dat2$S)]<-100
...
params<-list(
  ...
  # obs_sd=log(.1),
  cors=rep(.2,6),
  missing_R=rep(5,sum(is.na(SR_TMB_dat2$R))),
  test=0,
  # err=matrix(-1,n_yrs+1,4),
  # S_hat=(SR_TMB_dat$NOS+SR_TMB_dat$HOS)*.9,
  S_hat = SR_TMB_dat2$S+50,
  # R_hat=SR_TMB_dat$R*.9,
  # log_R=0
  logit_rho=rep(0,4)#,
  # alpha_mu=log(2),
  # alpha_sd=log(.5),
...
f<-function(parms){
  ...
  R2<-matrix(NA,nrow=nrow(R),ncol=ncol(R))
  cnt=1
  for(i in 1:nrow(R)){
    for(j in 1:ncol(R)){
      if(is.na(R[i,j])){
  ...
  err<-matrix(log(R2/pred),nrow=nrow(R),ncol=ncol(R))
  ...
  nll <- nll -sum(dexp(sigma,5,log=TRUE))#-sum(log_sigma)
  # 
  # 
  ...
  # nll <- nll -sum(dnorm(Rmax,Rmax_mu,exp(Rmax_sd),log=TRUE))
   ...
   nll <- nll -sum(dnorm(log(S_hat),log(S),.1,log=TRUE))
    # nll <- nll -sum(dnorm(exp(obs_sd),0,.05,log=TRUE))-obs_sd
      # nll <- nll -sum(dnorm(log(R_hat),log(R),.1,log=TRUE))
  # nll <- nll -sum(dnorm(logit_RRS,0,1,log=TRUE))
   ...
   #  
   nll <- nll -sum(dnorm(alpha,1,0.7,log=TRUE))
      nll <- nll -sum(dnorm(Rmax,10.82,exp(test),log=TRUE))
...
obj <- MakeADFun(f, params,random=c("missing_R","S_hat","alpha"))#,"R_hat""S_hat","Rmax"
...
f<-function(parms){
  ...
  nll <- nll -sum(dnorm(logit_rho,-1,1,log=TRUE))
  nll <- nll -sum(dexp(sigma,5,log=TRUE))
  #
  #
  # nll <- nll -sum(dnorm(alpha,alpha_mu,exp(alpha_sd),log=TRUE))
  # nll <- nll -sum(dnorm(Rmax,Rmax_mu,exp(Rmax_sd),log=TRUE))
  ...
  # nll <- nll -sum(dnorm(logit_RRS,0,1,log=TRUE))
   ...
   #
   # nll <- nll -sum(dnorm(alpha,1,0.7,log=TRUE))
      # nll <- nll -sum(dnorm(Rmax,11,.25),log=TRUE)
      # nll <- nll -sum(dexp(exp(test),10,log=TRUE))
  nll
}
...
# 
# #cosntruct model object
# obj <- MakeADFun(f, params,random=c("err"))#,"R_hat","S_hat"
# 
# #optimize
...
# (par_list$mu_RRS)
exp(par_list$test)
alpha<-exp(c(par_list$alpha,par_list$alpha_h))
Rmax<- c(exp(par_list$Rmax),1000000)
names(alpha)<-names(Rmax)<-rownames(SR_TMB_dat2$S)
...
ricker_pred<-SR_TMB_dat$test_mod_dat %>% group_by(basin,const,mu_rps,sd_rps,mu_s,sd_s,wild) %>% reframe(S=seq(0,max(max(S_tot),4000)*1.5,length.out=50)) %>%
  ...
  left_join(
    tibble(basin=c("Wenatchee","Methow","Okanogan","Hatchery"),
           marg_sd=marg_sd)
  ) |> 
  ungroup() |> 
    mutate(basin= fct_relevel(basin,rownames(SR_TMB_dat2$S)))
...
ricker_pred2 <-ricker_pred |> 
  mutate(,  
    ...
    # Rmax_vecpar_list=model.matrix(const~basin-1,data=ricker_pred)%*%c(Rmax)
    ) |> 
  rowwise() |> 
  mutate(
...
if(sim_for_model){
  ...
  SR_err_fun_TMB<-function(t1=fit_rep$err2 |> tail(1) |> c(),# AR1 random effect in final year
    ...
    proc_err[,1,]<-t1*rho + innovation_sd*err[,1,]
    for (i in 2:n_years){
