...
<p style="color:red">**This document does not represent agency policy positions nor management decisions, has not been endorsed by any agency, and should not be interpreted as such.**</p>


# Overview

This document describes the process of developing a simulation model for the purpose of informing decisions about harvest management strategies for the Summer Chinook management stock in the Columbia River. That simulation model can be accessed [here](https://marksorel.shinyapps.io/Summer_Chinook_simulator/). Underlying code can be accessed [here](https://github.com/marksorel8/Sumsimmer).
...
library(rRMIS)
releases<-get_release_data(first_by=1980, last_by=2023)
...
esc_dat<-
  ...
  #prior to 2012, the estimate of brodstock collection for the Methow include both Methow and Okanogan (colleciton atWells Dam). Goin to apportion the removals based on natural_origin spawning escapement.
...
esc_dat %>% filter(!is.na(`SG No. Age 4`)) |> select(year,population_name ,Spawn=NOS,Broodstock=NOBroodStockRemoved) %>%
  drop_na() |> pivot_longer(c(Spawn,Broodstock),values_to = "NO_Return") %>% ggplot(aes(x=year,y=NO_Return,fill=name))+geom_bar(stat="identity")+facet_wrap(~population_name)+ylab("Nat. Origin Escapement")+theme(legend.position = "top")
...
#   pivot_wider(names_from = age,values_from = NO_river_mouth) |>
#   write_csv("NO_river_mouth.csv")
# 
# long_dat |> ungroup() |>  group_by(brood_year,age,basin) |> summarise(n=length(NO_river_mouth),NO_river_mouth=sum(NO_river_mouth))|> 
#   pivot_wider(names_from = age,values_from = NO_river_mouth) |>
#   write_csv("NO_river_mouth_by_stock.csv")
# 
# esc_dat |> left_join(term_ER,by=c("year"="Year")) |> mutate(NOR_river_mouth=(NOR/(1-term_UM_HR))) |> group_by(year) |> summarise(NOR_river_mouth=sum(NOR_river_mouth),n=n()) |> filter(n==3) |> 
#   write.csv("NO_river_mouth_no_age.csv")
# 
# esc_dat |> left_join(term_ER,by=c("year"="Year")) |> mutate(NOR_river_mouth=(NOR/(1-term_UM_HR))) |> select(year,population_name,NOR_river_mouth) |> 
#   write.csv("NO_river_mouth_no_age_by_stock.csv")
# 
# esc_dat |> left_join(term_ER,by=c("year"="Year")) |> mutate(NOR_river_mouth=(NOR/(1-term_UM_HR))) |> group_by(year) |> summarise(NOR_river_mouth=sum(NOR_river_mouth),n=n()) |> filter(n==3) |> ggplot(aes(x=year,y=NOR_river_mouth))+geom_point()+geom_line()+ylim(
#   0,40000)
# 
# long_dat |> filter(age>3) |> group_by(spawn_year,basin) |> summarize(NOR=sum(NO_returns)) |> ungroup() |> ggplot(aes(x=spawn_year,y=NOR,color=basin))+geom_point()+geom_line()
# 
# long_dat |> filter(age>3) |> group_by(spawn_year) |> summarize(NOR=sum(NO_returns),n=n()) |> ungroup() |> filter(n>=8) |> ggplot(aes(x=spawn_year,y=NOR))+geom_point()+geom_line()
# 
# 
plotting_past_year_df<-pfmc_HRs |> filter(Year>=2008) |> select(Year,RMRS=`River Mouth Run Size`,`PFMC NT Ocean Impacts`,`PFMC Ocean Abundance`) |>
  left_join(
...
internal_data$plotting_past_year_df<-plotting_past_year_df
...
```{r esc_age, fig.cap="Figure 2. Annual proportions of carcasses of different ages recovered on the spawning grounds.", fig.width=8}
...
# returns |>mutate(prop_wid=wild/(hatchery+wild))|> ggplot(aes(x=year,y=prop_wid))+geom_col()+ylab("Proportion wild")+xlab("")+theme_minimal()
...
$$\text{recruits}_y=\alpha*\text{spawners}_y*\text{exp}\left(\frac{-\alpha*\text{spawners}_y}{e*\text{Rmax}}+\varepsilon_y\right)$$
where $\alpha$ represents the maximum recruits per spawner as you approach zero spawners, and $\text{Rmax}$ represents the maximum expected recruits and $e$ is Euler's number. The $\varepsilon$ term represents annual deviations from the Ricker relationship. The distribution of those deviations is described below. Hatchery-origin spawners were assumed to contribute to recruits at 80% of natural-natural origin spawners (i.e., a relative reproductive success of 0.8).


<!-- For the per-capita reproductive contribution of a hatchery-origin spawner relative to a natural origin spawner, I used the 10-year running average of proportionate natural influence (PNI) through the the brood year 4 years prior to the current year: -->

<!-- $$\text{spawners}_y = \text{NOS}_y+\overline{\text{PNI}}_y* \text{HOS}_y$$ -->

<!-- where $\overline{\text{PNI}}_y$ is the 10-year running average of proportionate natural influence (PNI) through the the brood year 4 years prior to the current brood year. -->
...
rel2<-releases %>%   
  filter(species==1 & run==2,rearing_type =="H",rmis_domain!="AK") |>  filter(psc_region%in%c("CECR","CRGN","UPCR","WAGN"),release_site!="QUILLAYUTE+TULALIP C",brood_year>=1980) |>
...
rel2 |> filter(brood_year>=1980)|> ggplot(aes(x=brood_year,y=releases,fill=lifestage))+geom_col()+ylab("Smolt releases")+xlab("Brood year")+theme_minimal()  + scale_y_continuous(labels = scales::unit_format(suffix="M",scale = 1e-6))+theme(axis.title.x =element_blank() ,text = element_text(size=18))+scale_fill_manual(values=  c("#D9B96E","#BED4F0"))+theme(legend.position = "top")
#ggsave("smolt_releases.png")
...
\end{gathered}
$$
where $\varepsilon_y$ represents the vector of errors for the four populations in a given year, $\phi$ determines the degree of autocorrelation and $\sigma=\sqrt{1-\phi^2}$. The annual increments were assumed to follow a multivariate normal distribution with an unstructured covariance matrix. Thus, when simulating future recruitment from this model, I accounted for how recruitment is correlated from year to year and among populations within years.


When simulating population trajectories, stock-recruitment model parameters for each simulation were drawn from a multivariate normal distribution defined by the maximum-likelihood estimates of parameters and the hessian matrix at the maximum likleihood estimate.
...
SR_dat<-esc_dat |> select(brood_year=year,basin=population_name,total_spawners,PHOS,NOS,HOS,pNI_bar) |> right_join(
  long_dat |> group_by(basin,brood_year) |> summarise(NO_recruits=sum(NO_recruits),n=n()) |>
    filter(n==5,brood_year<=2017)
) |> 
...
all_SR<-  
  SR_dat |> 
  mutate(S_RRS=NOS+HOS*.8,
         S_no=NOS,
         S_ho=HOS,
         S_dummy=1
         ) |> 
  select(basin,brood_year,S_tot=total_spawners,S_no,S_ho, S_RRS ,R=NO_recruits,PHOS,S_dummy,pNI_bar) |> 
  bind_rows(hatch_recruits |> 
              ...
              mutate(releases2=releases2/1000,
                     ...
                     S_dummy=1,
                     pNI_bar=0) |> 
              select(basin,brood_year,S_tot,S_no,S_ho, S_RRS,R=hatch_recruits,PHOS,S_dummy,pNI_bar)|> 
            filter(R<max(R,na.rm=T))
  ) |> 
  drop_na() |> 
  ungroup() |> 
  mutate(basin=fct_relevel(basin,c("Hatchery","Methow","Okanogan","Wenatchee"))) |> 
  arrange(basin)
...
make_TMB_data<-function(dat,spawners){
  ...
  S_mat<-
    dat |> 
    select(brood_year,{{spawners}},basin) |> 
    pivot_wider(names_from = basin,values_from = {{spawners}}) |> arrange((brood_year))|>
    select(-brood_year) |> 
    mutate(across(everything(),\(x)replace_na(x,99))) |>
    as.matrix() |> t()
  ...
  R_mat<-
    ...
    dat |> 
    select(brood_year,R,basin) |> 
    pivot_wider(names_from = basin,values_from = R)|> arrange((brood_year)) |> 
    select(-brood_year) |> as.matrix() |> t()
  ...
  pop_index<-dat$basin |> as.factor()|> as.numeric()
...
SR_TMB_dat<-make_TMB_data(all_SR #|> filter(brood_year>=1997)
                          ,S_RRS)#
...
params<-list(
  ...
  # obs_sd=log(.1),
  cors=rep(.2,6),
  missing_R=rep(5,sum(is.na(SR_TMB_dat$R_mat))),
  # test=rep(0,3),
  # err=matrix(-1,n_yrs+1,4),
  # S_hat=(SR_TMB_dat$NOS+SR_TMB_dat$HOS)*.9,
  S_hat = SR_TMB_dat$S_mat+50,#
   # R_hat=SR_TMB_dat$R_mat*.9,
  # log_R=0
  logit_rho=rep(0,4)#,
  # alpha_mu=log(2),
  # alpha_sd=log(.5)#,
...
f<-function(parms){
  ...
  alpha_vec<-exp(c(alpha_h,alpha))
  alpha_vec3<-alpha_vec*S_hat
  
  
  Rmax_vec2<-c(1000000,exp(Rmax))
  
  pred<-alpha_vec3*exp(-alpha_vec3/(exp(1)*Rmax_vec2)) #Ricker
  ...
  R2<-matrix(NA,nrow=nrow(R_mat),ncol=ncol(R_mat))
  cnt=1
  for(i in 1:nrow(R_mat)){
    for(j in 1:ncol(R_mat)){
      if(is.na(R_mat[i,j])){
        R2[i,j] <- exp(missing_R[cnt])
        cnt<-cnt+1
      }else{
        R2[i,j] <- R_mat[i,j]
      }
  ...
  err<-matrix(log(R2/pred),nrow=nrow(R_mat),ncol=ncol(R_mat))
  err2<-matrix(nrow=nrow(err),ncol=ncol(err))
  err2[,1]<-err[,1]
  for(i in 2:ncol(err2)){
    err2[,i]<-(err[,i] - rho*err[,i-1])/adj
  }
  t1<-err[,i-1] # error in last year, to be used in projection
  REPORT(t1)
    nll <- nll -sum(dmvnorm(t(err2),Sigma=us$corr(cors),scale=sigma,log=TRUE))


  # for ( i in 1:4){
  #   # for(j in 1:ncol(R_mat)){
  #   nll <- nll -sum(dgamma(((R2[i,])),shape= (pred[i,])/ sigma[i], scale=sigma[i],log=TRUE))
  #   # }
  # }
  ...
  nll <- nll -sum(dnorm(logit_rho,0,1,log=TRUE))
  nll <- nll -sum(dexp(sigma,2,log=TRUE))#-sum(log_sigma)
  # 
  # 
  # nll <- nll -sum(dnorm(alpha,alpha_mu,exp(alpha_sd),log=TRUE))
  # nll <- nll -sum(dnorm(Rmax,Rmax_mu,exp(Rmax_sd),log=TRUE))
  
  
  nll <- nll -sum(dnorm(log(S_hat),log(S_mat),.15,log=TRUE))
  # nll <- nll -sum(dnorm(exp(obs_sd),0,.05,log=TRUE))-obs_sd
  # nll <- nll -sum(dnorm(log(R_hat),log(R2),.15,log=TRUE))
  # nll <- nll -sum(dnorm(logit_RRS,0,1,log=TRUE))
  # nll <- nll -sum(dnorm(logit_RRS,mu_RRS,exp(sigma_RRS),log=TRUE))
  # nll <- nll  -sum(dexp(exp(sigma_RRS),1,log=TRUE))
  #  
  # nll <- nll -sum(dnorm(alpha,.92,0.25,log=TRUE))
  # nll <- nll -sum(dnorm(Rmax,10,.5,log=TRUE))
  # nll <- nll -sum(dexp(exp(test),1,log=TRUE))
  nll
}
...
obj <- MakeADFun(f, params,random=c("missing_R","S_hat","logit_rho"))#,"R_hat""S_hat"

#optimize
fit<-TMBhelper::fit_tmb(obj,newtonsteps =2,getJointPrecision  =TRUE)
...
var_list<-obj$env$parList(par=diag(fit$SD$cov.fixed))
# plogis(par_list$logit_RRS)
# exp(par_list$sigma_RRS)
# (par_list$mu_RRS)
# exp(par_list$test)
alpha<-exp(c(par_list$alpha_h,par_list$alpha))
Rmax<- c(1000000,exp(par_list$Rmax))
names(alpha)<-names(Rmax)<-rownames(SR_TMB_dat$S_mat)
beta<-alpha*exp(-1)/Rmax
...
a2<-exp(log(alpha)+(((marg_sd*sqrt(1-rho^2))^2)/(2*(1-rho^2))))
b2<-beta*(alpha/a2)

Smsy <-(1-W(exp(1-log(a2))))/b2
# Smsy <-Rmax*sqrt(1/alpha)-(Rmax/alpha) # beverton holt
sum(Smsy[2:4])
# Smsy
# fit$AIC


# Sumsimmer:::internal_data$alphas
# Sumsimmer:::internal_data$Rmax

covar_mat<-solve(fit$SD$jointPrecision)

set.seed(1234)
sim_SR_par<-mvtnorm::rmvnorm(500,obj$env$last.par.best,as.matrix(covar_mat))


alpha_posterior<-sim_SR_par[,grepl("alpha",colnames(sim_SR_par))]
Rmax_posterior<-sim_SR_par[,grepl("Rmax",colnames(sim_SR_par))]
rick_cor_posterior<-sim_SR_par[,grepl("cors",colnames(sim_SR_par))]
rho_posterior<-sim_SR_par[,grepl("rho",colnames(sim_SR_par))]
marg_sd_posterior<-sim_SR_par[,grepl("log_sigma",colnames(sim_SR_par))]

```



```{r plot_SR, fig.cap= "Figure. Stock-recruit relationships with 95% prediction intervals and past years' escapement estimates Vertical dashed line is at Smsy, the number of spawners for maximum sustainable yield."}
ricker_pred<-SR_TMB_dat$test_mod_dat %>% group_by(basin,const,mu_rps,sd_rps,mu_s,sd_s,wild) %>% reframe(S=seq(0,max(max(S_tot),4000)*1.25,length.out=50)) %>%
  mutate(year_fac=factor(3000)) |> 
  left_join(
    tibble(basin=names(alpha),
           marg_sd=marg_sd)
  ) |> 
  ungroup() |> 
  mutate(basin= fct_relevel(basin,rownames(SR_TMB_dat$S_mat)))
...
ricker_pred2 <-ricker_pred |> 
  mutate(,  
         ...
         Rmax_vec=model.matrix(const~basin-1,data=ricker_pred)%*%c(Rmax)
  ) |> 
  rowwise() |> 
  mutate(
    recruits=alpha_vec*S*exp((-alpha_vec*S)/(exp(1)*Rmax_vec)),
    # variance=(exp(marg_sd^2)-1)*exp(2*log(recruits)+marg_sd^2),
    # recruits=alpha_vec*S/(1+alpha_vec*S/Rmax_vec),
    lcl=
      # qgamma(.025,shape=recruits/marg_sd,scale=marg_sd),#
       # qgamma(.025,shape=recruits^2/variance,scale=variance/recruits),
    exp(log(recruits)-1.96*marg_sd),
    ucl=
      # qgamma(.975,shape=recruits/marg_sd,scale=marg_sd)#
    exp(log(recruits)+1.96*marg_sd)
      # qgamma(.975,shape=recruits^2/variance,scale=variance/recruits)#
  )
...
ricker_pred2 |> filter(basin!="Hatchery")|> ggplot(aes(x=S))+geom_ribbon(aes(ymax = ucl,ymin=lcl),fill="darkgrey")+
  facet_wrap(~basin,scales="free",nrow=2)+ geom_line(aes(y=recruits),color="blue",lwd=1.25)+geom_point(data=all_SR |> filter(basin!="Hatchery") |> mutate(S=S_RRS) ,aes(y=R))+
  # geom_point(data=all_SR |> filter(brood_year>=1997) |> ungroup() |>  mutate(S=par_list$S_hat) ,aes(y=R),col="red")+
  ylab("Recruits") +
  xlab("Spawners or 1,000 hatchery smolts released")+geom_vline(data=tibble(Smsy=c(Smsy[2:4]),basin=c("Methow","Okanogan","Wenatchee")),aes(xintercept=Smsy),linetype = 2)+
...
tibble(population=c("Hatchery","Methow","Okanogan","Wenatchee"),"$\\alpha$"=round(alpha,2),Rmax=c("",round(Rmax[2:4],0)),Smsy=c(2000,round(Smsy[2:4],0)),"$\\phi$"=round(rho,2),"Marginal sd"=round(marg_sd,2)) |> knitr::kable(caption = "Table 1. Mean parameter estimates for stock recruit models for each population. Smsy is the number of spawners at maximum sustainable yield. The Smsy for the hatchery population will be the broodstock target, which is about 2,000 hatchery-origin fish. *Marginal sd* is the standard deviation of the annual deviations around the relationship. Note that $\\alpha$ for the hatchery population is in terms of recruits per 1,000 yearling smolts.")
...
To account for [Allee effects](https://en.wikipedia.org/wiki/Allee_effect), I assumed that recruitment would be 25% of the expected value when there were <50 spawners in a basin. 
...
```{r age_sim_fig,fig.cap="Figure. One random simulated time series of recruit age proportions for brood years 2018-2067.The years through 2017 are estimated age proportions for those years."}
...
I model the AEQ mortalities in PFMC fisheries as a proportion of the River Mouth Run Size with log-normal error. The PFMC AEQ mortalities were assumed to average 8.2% of the River Mouth Run Size with `r round(exp(pfmc_mod_fit$par[2])*100)`% average inter-annual variability.
...
tibble(RMRS=RMRS,fit=.082*RMRS) |> mutate(
...
I implemented three different options for simulating in-river harvest  as a function of what is allowed. 
...
3. Finally, one can specify to have the harvest fluctuate around the allowed (i.e., exceeds allowed about half the years) with logrnomal error.
...
The fishing mortality on natural-origin fish is the fishing mortality on unmarked fish.  The fishing mortality on hatchery fish is the weighted average of the marked and unmarked harvest rates, where the weights are determined by the mark rate of the hatchery stock.

$$HR^\text{Hatchery}=p^\text{Mark hatchery}*HR^\text{marked}+(1-p^\text{Mark hatchery})*HR^\text{unmarked}$$


I assumed that the proportion of handled unmarked fish that will be released ($p^\text{UM release}$) in non-Treaty fisheries would be the average 2020--2023 rate (`r round(URR |> filter(Year>=2021) |> summarize(URR=mean(URR)) |> pull(URR),2)`), although this parameter can be varied in the model.
...
I assumed that the Methow and Wenatchee integrated programs will collect their target natural-origin broodstock, with 10% error, unless that constitutes >30% of the total natural-origin escapement to that tributary. In that case, 30% of the natural-origin escapement is collected, with 10% error. The escapement goals for the Wenatchee is 310 total adults and for the Methow is 122. 
...
```{r broodstock_removal, fig.cap="Natural-origin broodstock collection vs. natural-origin escapement by basin."}
...
phos_dat2 |> ggplot(aes(x=hatchery_escapement))+geom_ribbon(aes(ymax=pred_HOS+(1.96*resid_sd),ymin=pmax(0,pred_HOS-(1.96*resid_sd))),fill="gray")+geom_line(aes(y=pred_HOS))+geom_point(aes(y=HOS))+facet_wrap(~population_name,scales="free")+ylab("Hatchery-origin spawners")+xlab("Hatchery escapement")+
  scale_y_continuous(labels = scales::unit_format(suffix="K",scale = 1e-3))+
  scale_x_continuous(labels = scales::unit_format(suffix="K",scale = 1e-3))
```
...
if(sim_for_model){
  ...
  SR_err_fun_TMB<-function(t1=fit_rep$t1 |> c(),# AR1 random effect in final year
                           n_years=50,
                           n_sims=500){
    ...
    rick_cor_j<-us$corr(rick_cor_posterior[j,])
    rho_j<-plogis(rho_posterior[j,])
    marg_sd_j<-exp(marg_sd_posterior[j,])
    # random deviates
    err<-(mvtnorm::rmvnorm(n=n_years,sigma=rick_cor_j))
    innovation_sd<-marg_sd_j*sqrt(1-rho_j^2)
    
    proc_err[,1,j]<-t1*rho_j + innovation_sd*err[1,]
    for (i in 2:n_years){
      proc_err[,i,j]<-proc_err[,i-1,j]*rho_j + innovation_sd*err[i,]
    }
    }
  ...
  set.seed(1234)
  SR_err<-SR_err_fun_TMB(t1 = fit_rep$t1 |> c(),n_years = 50, n_sims = 500)
  
  
  
  internal_data$alphas<-exp(alpha_posterior)
  # internal_data$betas<-beta
  internal_data$Rmax<-exp(Rmax_posterior)
  ...
  internal_data$PFMC_ave_prop<-.076/(1-.076)#pfmc_mod_fit$par[1]
  ...
  if(add_baselines){
    sim<-pop_sim()
    sim_list<-list(perf=summarize_sim(sim,HCR="Current"))#hcr=seq_HCR(),
    
    internal_data$`Current_MA`<-sim_list
    
    
    internal_data$`No_harvest`<-list(perf=summarize_sim(pop_sim(treaty_tiers = NA,treaty_rates = 0,treaty_scalar=NA,treaty_offset=NA,treaty_share=NA,NT_tiers=NA,NT_rates=0,NT_scalar=NA,NT_offset=NA,NT_share=NA,PFMC_include_above=1000000),HCR="No harvest"))#,hcr=seq_HCR(treaty_tiers = NA,treaty_rates = 0,NT_tiers=NA,NT_rates=0,PFMC_include_above=1000000))
    ...
    #                PFMC_include_above=0)
    # 
    # 
    # internal_data$`PST` <-list(perf=summarize_sim(do.call(pop_sim,PST_HCR),HCR="PST"))#,hcr=do.call(seq_HCR,PST_HCR))
    
    internal_data$`Fixed_20%` <-list(perf=summarize_sim(pop_sim(treaty_tiers = NA,treaty_rates = 0.2,treaty_scalar=NA,treaty_offset=NA,treaty_share=NA,NT_tiers=NA,NT_rates=0.2,NT_scalar=NA,NT_offset=NA,NT_share=NA,PFMC_include_above=0),HCR="1) Fixed 40%"))#,hcr=do.call(seq_HCR,PST_HCR))

    # New_AB_HCR<-list(treaty_tiers=c(20000,50000,rep(NA,2)),
    #                treaty_rates=c(.1,.2,rep(NA,2)),
    #                treaty_scalar=c(NA,NA,.75,rep(NA,1)),
    #                treaty_offset=c(NA,NA,17500,rep(NA,1)),
    #                treaty_share = c(NA,NA,.5,rep(NA,1)),
    #                NT_tiers=c(20000,50000,rep(NA,2)),
    #                NT_rates=c(.1,.2,rep(NA,2)),
    #                NT_scalar=c(NA,NA,.75,rep(NA,1)),
    #                NT_offset=c(NA,NA,17500,rep(NA,1)),
    #                NT_share = c(NA,NA,.5,rep(NA,1)),
    #                PFMC_include_above=0)
    # 
    New_AB_HCR<-list( treaty_tiers=c(16000/(1-.076),29000/(1-.076),rep(NA,3)),
                      treaty_rates=c(.05,.1,rep(NA,3)),
                      treaty_scalar=c(NA,NA,.75,rep(NA,2)),
                      treaty_offset=c(NA,NA,16500,rep(NA,2)),
                      treaty_share = c(NA,NA,.5,rep(NA,2)),
                      NT_tiers=c(16000/(1-.076),29000/(1-.076),rep(NA,3)),
                      NT_rates=c(.05,.1,rep(NA,3)),
                      NT_scalar=c(NA,NA,.75,rep(NA,2)),
                      NT_offset=c(NA,NA,16500,rep(NA,2)),
                      NT_share = c(NA,NA,.5,rep(NA,2)),
                      PFMC_include_above=0)
    
    
        internal_data$`New_AB_tot` <-list(perf=summarize_sim(do.call(pop_sim,New_AB_HCR),HCR="Proposed"))#,hcr=do.call(seq_HCR,PST_HCR))
        
        
        #     New_AB_HCR2<-list(treaty_tiers=c(20000,32500,50000,rep(NA,2)),
        #            treaty_rates=c(.1,.12,.2,rep(NA,2)),
        #            treaty_scalar=c(NA,NA,NA,.75,rep(NA,1)),
        #            treaty_offset=c(NA,NA,NA,17500,rep(NA,1)),
        #            treaty_share = c(NA,NA,NA,.5,rep(NA,1)),
        #            NT_tiers=c(20000,32500,50000,rep(NA,2)),
        #            NT_rates=c(.1,.12,.2,rep(NA,2)),
        #            NT_scalar=c(NA,NA,NA,.75,rep(NA,1)),
        #            NT_offset=c(NA,NA,NA,17500,rep(NA,1)),
        #            NT_share = c(NA,NA,NA,.5,rep(NA,1)),
        #            PFMC_include_above=0)
        # 
        # 
        # internal_data$`Option 3` <-list(perf=summarize_sim(do.call(pop_sim,New_AB_HCR2),HCR="Option 3"))#,hcr=do.call(seq_HCR,PST_HCR))
        # 
        
        
          # 
          # New_AB_NO_HCR<-list(index="wild",
          #                    treaty_tiers=c(4000,10000,rep(NA,2)),
          #          treaty_rates=c(.1,.2,rep(NA,2)),
          #          treaty_scalar=c(NA,NA,.75,rep(NA,1)),
          #          treaty_offset=c(NA,NA,3500,rep(NA,1)),
          #          treaty_share = c(NA,NA,.5,rep(NA,1)),
          #          NT_tiers=c(4000,10000,rep(NA,2)),
          #          NT_rates=c(.1,.2,rep(NA,2)),
          #          NT_scalar=c(NA,NA,.75,rep(NA,1)),
          #          NT_offset=c(NA,NA,3500,rep(NA,1)),
          #          NT_share = c(NA,NA,.5,rep(NA,1)),
          #          PFMC_include_above=0)



         # internal_data$`New_AB_nat` <-list(perf=summarize_sim(do.call(pop_sim,New_AB_NO_HCR),HCR="Option 2"))#,hcr=do.call(seq_HCR,PST_HCR))
    ...
    # internal_data$`No harvest`<-list(perf=summarize_sim(pop_sim(treaty_tiers = NA,treaty_rates = 0,NT_tiers=NA,NT_rates=0),HCR="No_harvest"),hcr=seq_HCR(treaty_tiers = NA,treaty_rates = 0,NT_tiers=NA,NT_rates=0))
    
    # CRITFC_counter<-list( treaty_tiers=c(16000,36250,50000,Inf,rep(NA,3)),
    ...
    #               NT_rates=c(200,200,200,200,200,NA,NA),
    #               NT_scalar=c(rep(NA,5),1,.75),
    #               NT_offset=c(rep(NA,5),29000,16500),
    #               NT_share=c(rep(NA,5),.5,.5),
    #               PFMC_include_above=36250)
    
     # 
     # internal_data$`CRITFC_counter` <-list(perf=summarize_sim(do.call(pop_sim,CRITFC_counter),HCR="Tribes proposed"))
...
As a check on whether the simulation model passes the red face test, I compared the results of 25 year simulations using the current harvest control rule, to observations from 2008-2022, when the current harvest control rules was been in effect. I wouldn't expect the simulations to match the past observations perfectly, because the past 15 years won't necessarily reflect the average state of the system. In other words, the productivity over the last 15 years (e.g, due to ocean conditions) could have been above or below average, whereas the simulations should reflect the average.

Note that when considering the outcomes of alternative harvest control rules, we want to compare performance metrics across simulations, rather than comparing them to historical estimates. That is because simulations all assume the same environmental and biological conditions. Comparing to estimates from the past would not be appropriate because those occurred under a different set of environmental conditions.
...
```{r, fig.cap="Figure. Estimates of 2008-2022 in-river harvest (bars), median of simulated future harvest trajectories (thick black line), random individual simulated trajectory (thin black line), and 95% prediction interval of future harvest (shaded area).", fig.width=8}
plot_harvest_trajectory(harv_traj_dat_fun(sim))
```


### Harvest sharing ratio
```{r ratio_obs, fig.cap="Figure. quantiles of the ratio of Treaty and non-Treaty harvest (inclusive of ocean PFMC AEQ) for both the 2008-2022 period and simulations of the 2023-2047 period."}
...
```{r fig.cap="Figure. Estimates of 2008-2022 natural-origin escapement (bars)", fig.width=8}
...
# Simulation of proposed harvest control rules

```{r perf_plots}
rm(internal_data)

perf_list<-list(internal_data$No_harvest$perf[1:10],
                internal_data$Current_MA$perf[1:10],
                internal_data$New_AB_tot$perf[1:10]#,

                # internal_data$`Fixed_20%`$perf[1:10],
                # internal_data$New_AB_nat$perf[1:10],
                # internal_data$`Option 3`$perf[1:10]

)

col_vec<-c(
  "#D55E00" ,"#000000","#0072B2","#009E73","#E69F00", "#56B4E9"
)

p3<-plot_all_fun(perf_list,guide_rows=1,colors_vec = col_vec)
# p3$QET_plot
# ggsave(here("plots","compare_pQET.png"))
```


```{r perf_plots_NOE, fig.cap = "Boxplots of natural-origin escapement, which includes spawning, hatchery broodstock, and hatchery surplus. The whiskers of the boxplot span 95% quantile intervals, the box spans the 50% quantile interval, and the midline is the median. Quantiles were calculated across simmulations. Dashed lines represent minimum abundance thresholds of 1,000 in the Wenatchee and Methow, 2,000 in the Okanogan, and 12,143 for the total."}
p3$NOE_plot
# ggsave(here("plots","compare_mean_escapement.png"))
```

```{r perf_plots_MAT, fig.cap= "Proportion of simulations in which the mean escapement was less than a minimum abundane threshold of 1,000 adults in the Wenatchee and Methow, 2,000 in the Okanogan, and 4,000 for these three natural-origin populations combined." }
p3$MAT_plot
# ggsave(here("plots","compare_MAT.png"))

```

```{r perf_plots_harv, fig.cap = "Boxplots of mean annual harvest across 25-year simulations. The whiskers of the boxplot span 95% quantile intervals, the box spans the 50% quantile interval, and the midline is the median. Quantiles were calculated across simmulations."}
p3$Mean_harv_plot
# ggsave(here("plots","compare_mean_harv.png"))
```


```{r perf_plots_low_harv, fig.cap = "Boxplots of mean annual harvest in the years with the lowest 20% of run sizes. This represents the five lowest run size years out of the 25 year simulations. The whiskers of the boxplot span 95% quantile intervals, the box spans the 50% quantile interval, and the midline is the median. Quantiles were calculated across simmulations."}
p3$Low_harv_plot
# ggsave(here("plots","compare_low_harv.png"))
```
