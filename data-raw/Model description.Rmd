...
# Disclaimer
 ...
 \textcolor{red}{**This document does not represent any policy policy position nor management decision of any agency, has not been endorsed by any agency, and should not be interpreted as such.**}
...
$$\text{recruits}_y=\alpha*\text{spawners}_y*\text{exp}(-\beta*\text{spawners}_y+\varepsilon_y)$$
...
I fit three different versions of the model representing different scenarios of how hatchery-origin spawners contribute to production.

1) Hatchery-origin spawners were included in the total number of spawners in these relationships, which assumes that hatchery- and natural-origin spawners have similar reproductive success. 
2) Hatchery origin spawners were not included, which assumes that hatchery-origin spawners have zero reproductive success
3) the total number of spawners was calculated as:

$$\text{spawners}_y = \text{NOS}_y+\delta_y ~\text{HOS}_y$$

where $\delta_y = .1+0.6~\overline{\text{PNI}}_y$ and $\overline{\text{PNI}}_y$ is the 5-year running average of proportionate natural influence (PNI) through the the brood year 4 years prior to the current year. This specification is designed to scale the reproductive success of hatchery-origin spawners based on PNI such that their minimum is 10% of a natural-origin spawner and their maximum is 70%. 
...
I fit a linear relationship between hatchery smolt releases and hatchery returns. This assumes that there is no density dependent survival after release from the hatchery (i.e., $\beta =0$ in the Ricker model defined above). The majority of smolts have been released at the yearling lifestage. I counted subyearling releases as 0.5 of a yearling release when calculating the total number of "yearling equivalent" releases to fit the model to. 
...
SR_dat<-esc_dat |> select(brood_year=year,basin=population_name,total_spawners,PHOS,NOS,HOS) |> right_join(
...
all_SR<-  
  SR_dat |>
  select(basin,brood_year,S_tot=total_spawners,S_no=NOS,S_PNI=NOS+HOS*,R=NO_recruits,PHOS) |> 
