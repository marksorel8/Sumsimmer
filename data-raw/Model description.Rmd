...
PreTermSurv_mod2<-t(fit.dfa.surv$ytT*pre_turm_surv_scales$sd+pre_turm_surv_scales$mu)|> `colnames<-`(rownames(wide_surv)) |> as_tibble() |>  mutate(cy=1973:2022) |> pivot_longer(-cy,names_to = "Age",values_to = "logit_surv") |> mutate(age=as.numeric(substr(Age,4,4)),by=cy-age) |> 
  arrange(by,Age) |> 
...
sim_MREER<-t((cbind(fit.dfa.surv$ytT,sim_MREER[,,1])*pre_turm_surv_scales$sd+pre_turm_surv_scales$mu))|> `colnames<-`(rownames(wide_surv)) |> as_tibble() |>  mutate(cy=seq(from=1973,by=1,length.out=100)) |> pivot_longer(-cy,names_to = "Age",values_to = "logit_surv") |> mutate(age=as.numeric(substr(Age,4,4)),by=cy-age) |> 
  arrange(by,Age) |> 
  group_by(by) |> mutate(n=n(),MREER=1-cumprod(plogis(logit_surv))) |> 
  filter(n>=age-1) |> 
  ungroup() 
...
tac_RR |> pivot_longer(Age3:Age6,names_to="Age",values_to="riv_mouth") |>
  ...
  geom_point()+geom_line()+ylab("Ocean ER")+xlim(1998,2022)+theme_minimal()
...
The result of the run reconstruction was an estimate of hatchery and natural-origin recruits by age and year. The primary data time series limiting the length of the time series of recosntructed recruits was that of the escapement age compositon. This could be imputed (e.g., within and integrated model) if needed in the future.
...
- Terminal harvest rates as a function of harvest control rule
...
$$recruits_y=\alpha*spawners_y*\text{exp}(-\beta*spawners_y+\varepsilon_y)$$
...
esc_dat |> filter(!is.na(`SG No. Age 4`),year<=2022)|> select(year,basin=population_name,hatchery=HOS,wild=NOS) |> pivot_longer(c(hatchery,wild),names_to = "Origin",values_to = "Spawners")|> ggplot(aes(x=year,y=Spawners,fill=Origin))+geom_col()+facet_grid(~basin)+theme_minimal()
```


I fit a linear relationship between hatchery smolt releases and hatchery returns. This assumes that there is no density dependent survival after release from the hatchery (i.e., $\beta =0$ in the Ricker model defined abvoe). The majority of smolts have been released at the yearling lifestage. I counted subyearling releases as 0.5 of a yearling release when calculating the total number of "yearling equivalent" releases to fit the model to. 

```{r releases, fig.cap="igure .Upper Columbia River hatchery smolt releases by lifestage and brood year.",cache=TRUE}
...
```{r plot_SR, fig.cap= "Figure. Stock-recruit relationships with 95% prediction intervals and past-years' observations."}
...
ricker_pred |> ggplot(aes(x=S))+geom_ribbon(aes(ymax = ucl,ymin=lcl),fill="grey")+facet_wrap(~basin,scales="free")+ geom_line(aes(y=recruits),color="blue",lwd=1.25)+geom_point(data=all_SR ,aes(y=R))+
  ylab("Recruits") +
  xlab("Spawners or 1,000 hatchery smolts released")+theme_minimal()
...
We assumed that the geometric mean number of yearling smolt equivalents released since year 2000 (`r (round(exp(smolts_mu)*1000,0))`) would be released annually, with lognormal error (CV = `r (round(smolts_sd,2))`), as long as the hatchery-origin escapement was sufficient to provide 2000 fish for broodstock, plus any broodstock that was needed to back fill for natural-origin broodstock targets in conservation programs. The number of smolts released was assumed to decrease proportionally to any shortfall in hatchery escapement for broodstock.

```{r}

rel2 |> filter(brood_year >=1995) |> ggplot(aes(x=brood_year,y=releases2))+geom_col()+ylab("Smolt releases")+xlab("Brood year")+theme_minimal()
```
...
like the autocorrelated residual term in the spawner-recruit model, to represent the effects of common environmental factors on age proportions. By allowing each age and population to load onto that factor with a unique parameter $\zeta_{i,a}$, the age proportions could have positive covariance, as could result from common envrionmental experiences having similar affects among populations, and negative covariance, such as could occur if the older age proportions increase at the same time that the younger age proportions decrease. 
...
mat_age_dat |>  group_by(basin,brood_year) |> 
  mutate(mean_age=sum(age*by_props)) |> ggplot(aes(x=brood_year,y=mean_age))+geom_point()+geom_line()+facet_wrap(~basin)+geom_smooth()+theme_minimal()
...
sim1 |> filter(brood_year>2016) |> bind_rows(mat_age_dat |> mutate(age=as.factor(age)) |> filter(brood_year<=2016)) |> 
  ggplot(aes(x=brood_year,y=by_props,fill=age)) +
  geom_col() +
  facet_wrap(~basin)+ylab("Recruit age props")+geom_vline(aes(xintercept=2016))+theme_minimal()
...
Similarly to age proportions, we modeled annual and age-specific ocean fisheries mortality allowing for correlation among successive years as well as among ages within years. 

We assumed that the ocean exploitation would be similar to the most recent 10 years, and will not return to the consistently higher explotation rates observed prior to the 1990s. 


```{r mreer_sim, fig.cap="Figure. Mature run exploitation rates (MREER) by age and brood year. The years to the left the vertical black line are observation and the years to the right are one random simulation of future ocean fishing exploitation." }
...
In the 2019-2028 USvOR Management Agreement, total allowable catch is determined based on the River Mouth Run Size and, for the non-Treaty share, the Adult Equivalent (AEQ) mortalities from PFMC fisheries during the calendar year. 

#### PFMC AEQ

We model the AEQ mortalities in PFMC fisheries as a proportion of the River Mouth Run Size with log-normal error.

```{r PFMC, fig.cap="PFMC AEQ mortalities vs. river mouth run size. The line and shaded area represent the mean and 95% prediction intervals. The points represent observations -- from TAC run-reconstruction files -- from 2010-2021."}
pfmc_mort_dat<-
...
1. The default model assumes that total harvest has a linear relationship with allowed harvest in log space. Both the allowed and the realized include the PFMC AEQ mortalities. The 2019 fishery was removed when fitting this model due its outsized effect on the residual error standard deviation. 

2. Since 2008, both sectors have been more likely to exceed their allowable take when it is small. The most dramatic example of this was the 2019 non-Treaty in-river fishery. Not fully understanding whether this will be a persistent pattern that goes into the future, I developed an alternative model where the average realized harvest rate is constant below a certain threshold of allowed harvest. This model would reflect a world where harvest is not responsive to what is allowed below a certain threshold.
...
The above models can be used to simulate harvest rates (including incidental mortality) on the run at large. That is the metric used in the current harvest control rule. But, harvest rates differ for wild and hatchery components due to mark selective fisheries. Three additional parameters are needed to convert harvest rates on the aggregate to harvest on the hatchery and wild components, assuming that marked and unmarked fish are handled at the same rate.  

1) $p^\text{UM release}$ -- The proportion of unmarked fish handled in the fishery, which are released.
2) $p^\text{Mark hatchery}$ --The mark rate of the hatchery return.
3) $p^\text{Mort}$ -- The mortality rate of fish after being released. This is assumed to equal 0.15, which is the TAC approved rate for sport fisheries.


The handle rate, or the proportion of the return captured in a fishery, can be calculated as,

$$Handle~rate = \frac{Harvest~rate^\text{Total}}{1-\left[p^\text{UM release}(1-p^\text{Mark total})(1-p^\text{Mort})\right]}$$
where $p^\text{Mark total}$ is the mark rate of the total run, which is calculated as the hatchery mark rate times the hatchery run size, devided by the total river mouth run size including natural-origin fish. Note that the denominator in the above equation is the proportion of handled fish that do not survive (i.e., the probability that a handled fish is not unmarked, released, and subsequently survives). 

The harvest rate of the unmarked fish, which is the fishing mortality rate of the natural-origin populations, is then,

$$Harvest~rate^\text{Unmarked} = Handle~rate\left[1- \left(p^\text{UM release}(1-p^\text{Mort}) \right)\right]$$

Note that the second term is the probability that an unmarked fish is not released and subsequently survives. 

The harvest rate of hatchery fish is then the weighted average of the handle rate (which is also the harvest rate of marked fish assuming no release of marked fish) and the unmarked harvest rate, where the weights are determined by the mark rate of the hatchery stock.

$$Harvest~rate^\text{Hatchery}=p^\text{Mark hatchery}*Handle~rate+(1-p^\text{Mark hatchery})*Harvest~rate^\text{Unmarked}$$

The assumption of equal capture probability of marked and unmarked fish would break down as mark-selective fisheries change the ratio of marked and unmarked fish at large. More complex math to deal with this could be implemented in the future.



I assumed that the proportion of handled unmarked fish that will be released ($p^\text{UM release}$) in non-Treaty fisheries will be the average 2020--2023 rate (`r round(URR |> filter(Year>=2021) |> summarize(URR=mean(URR)) |> pull(URR),2)`)
...
I assumed that the mark rate of hatchery fish would be the average mark rate of smolt releases from brood years 2008--2021, excluding an anomolous year in 2019 (mark rate =`r round(ave_MR["mean(MR)"],2) `)


### Broodstock collection

We assume that the Methow and Wenatchee integrated programs will collect their target natural-origin broodstock, with 10% error, unless that constitues >30% of the total natural-origin escapement to that tributary. In that case, 30% of the natural-origin escapement is collected, with 10% error. The escapement goals for the Wenatchee is 310 total adults and for the Methow is 122. 
...
pHOS was modeled as a decreasing function of log transformed natural-origin spawner abundance and hatchery escapement, with normal errors in logit-space. Each basin has it's own unique intercept and effect of natural-origin spawners. 


```{r pHOS, fig.cap="Figure. proportion of hatchery origin spawners (pHOS) vs natural-origin spawner abundance. THe lines represent the mean relationship at different levels of hatchery escapement, and the points represent observations from 2010-2022."}
...
pHOS_mod<-glmmTMB::glmmTMB(logit_phos~population_name+log(NOS):population_name+log(hatchery_escapement)-1+diag(population_name+0|year_fac),data=phos_dat|> filter(!is.na(hatchery_escapement)),dispformula = ~0)
...
pdat<-
  ...
  dplyr::select(population_name, NOS = value_sequence) |> expand_grid(hatchery_escapement=seq(10000,80000,by=2000),year_fac=as.factor(3000))

pred<-predict(pHOS_mod,newdata = pdat)
pdat<-pdat |> mutate(pred=plogis(pred),
                     group=as.factor(hatchery_escapement)) 


pdat|> ggplot(aes(x=NOS,y=pred,color=hatchery_escapement))+geom_line(aes(group=group),lwd=2)+facet_wrap(~population_name,scales="free_x")+scale_color_gradient(low = "lightblue", high = "darkblue",limits=c(10000,80000))+ylab("pHOS")+xlab("Natural origin spawners")+geom_point(data=phos_dat |> filter(!is.na(hatchery_escapement)),aes(x=NOS,y=PHOS,fill=hatchery_escapement),size=3,shape=21,color="black")+scale_fill_gradient(low = "lightblue", high = "darkblue",limits=c(10000,80000)) +theme_minimal()
...
# deciding whether to log transform NOS and hatchery returns could have considerable effect on pQET type outcomes. Need to chat with managers to determine how to represent managment of pHOS. Key question is whether pHOS increases a lot at low NOS (log relationship) or not.
```
...
if(sim_for_model){
  ...
  # fill missing 2022 hatchery releases with mean 
  set.seed(2021)
  init_S$Hatchery[6]<-exp(smolts_mu)
...
# else{
#   internal_data<-read_file(here::here("R","sysdata.rda"))
# }
...
# Historical vs. simulation comparison plots
...
yrs<-7:31
  ...
  pfmc_morts<-
    internal_data$pfmc_morts_2 |> select(Year,Sector,In_river_harvest) |> pivot_wider(values_from = In_river_harvest,names_from = Sector) |> left_join(internal_data$pfmc_morts_2 |> select(Year,Sector,Total_harvest) |> filter(Sector=="NT") |> select(Year,NT_total_harvest=Total_harvest)) |>  filter(between(Year,2008,2022)) |>
    `colnames<-`(c("Year","Total Treaty", "Total In-river NT","Total NT"))


  harvest_quants<-data.frame(rbind(
    apply(apply(apply(sim$terminal_NT[,yrs,],2:3,sum),2,quantile,c(.025,.25,.5,.75,.975)),1,geo_mean), # sum across populations, quantile across years, geometrtic mean across simulations
    apply(apply(apply(sim$terminal_NT[,yrs,],2:3,sum) + sim$PFMC[yrs,],2,quantile,c(.025,.25,.5,.75,.975)),1,geo_mean),
    apply(apply(apply(sim$terminal_treaty[,yrs,],2:3,sum),2,quantile,c(.025,.25,.5,.75,.975)),1,geo_mean)
  ))
...
bind_rows(
internal_data$esc_dat %>% filter(between(year,2008,2022)) |> 
