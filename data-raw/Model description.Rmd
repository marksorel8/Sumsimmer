...
I fit a linear relationship between hatchery smolt releases and hatchery returns. This assumes that there is no density dependent survival after release from the hatchery (i.e., $\beta =0$ in the Ricker model defined abvoe). The majority of smolts have been released at the yearling lifestage. I counted subyearling releases as 0.5 of a yearling release when calculating the total number of "yearling-equivalent" releases to use in the model. 
...
like the autocorrelated residual term in the spawner-recruit model, to represent the effects of common environmental factors on age proportions. By allowing each age and population to load onto that factor with a unique parameter $\zeta_{i,a}$, the age proportions could have negative covariance, such would occur if the older age proportions increase at the same time that the younger age proportions decrease. 
...
```{r PFMC, fig.cap="PFMC AEQ mortalities vs. river mouth run size. The line adn shaded area represent the mean and 95% prediction intervals. The points represent observations -- from TAC run-reconstruction files -- from 2010-2021."}
pfmc_mort_dat<-
...
mod_2_plot<-in_riv_pred2 |> 
   ggplot(aes(x=in_river_allowed_HR))+geom_ribbon(aes(ymin=lcl,ymax=ucl),fill="grey")+geom_line(aes(y=pred),lwd=1.25,color="firebrick4")+geom_point(data=pfmc_morts_2,aes(y=in_river_HR),cex=1.75)+geom_abline(intercept=0,slope=1,lwd=.8,col="blue")+facet_wrap(~Sector)+ylab("In-rive HR")+xlab("Allowed in-river HR")+theme_classic()
...
The above models can be used to simulate harvest rates (including incidental mortality) on the run at large, but harvest rates differ for wild and hatchery components due to mark selective fisheries. Three additional parameters are needed to convert harvest rates on the aggregate to harvest on the hatchery and wild components, assuming that marked and unmarked fish are handled at a constant rate.  
...
3) $p^\text{Mort}$ -- The mortality rate of fish after being released.
4) $p^\text{Mark total}$ --The mark rate of the total run. This can be calculated from the mark rate of the hatchery return and the return to the river mouth of hatchery and wild stocks.

$$Handle~rate = \frac{Harvest~rate^\text{Total}}{1-\left[p^\text{UM release}(1-p^\text{Mark total})(1-p^\text{Mort})\right]}$$

Note that the denominator in this equation is the probability that a handled fish is not unmarked, released, and survived. 

The harvest rate of the unmarked fish, which applies to the natural-origin fish, is then,

$$Harvest~rate^\text{Unmarked} = Handle~rate\left[1- \left(p^\text{UM release}(1-p^\text{Mort}) \right)\right]$$

Note that the second term is the probability that an unmarked fish is not released and survives. 
...
We assumed that the geometric mean number of yearling smolt equivalents released since year 2000 (`r round(exp(mean(rel3 |> filter(brood_year >=2000) |> pull(log_rel)))*1000,0)`, $CV = $ `r round(sd(rel3 |> filter(brood_year >=2000) |> pull(log_rel)),2)`) would be released annually, with lognormal error) as long as the hatchery-origin escapement was sufficient to provide 2000 fish for broodstock, plus any broodstock that was needed to back fill for natural-origin broodstock targets in conservation programs. The number of smolts released would decrease proportionally to any shortfall in hatchery escapement for broodstock.
...
rel_mod<-lm(log_rel~1,(rel3 |> filter(brood_year >=2000) |> pull(log_rel)))
